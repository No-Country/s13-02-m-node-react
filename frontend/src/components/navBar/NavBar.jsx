  ...
  return (
    <AppBar position="static" className="flex">
      <Container maxWidth="xl" className="bg-[#10151D] h-20">
        <Toolbar disableGutters className="flex content-center place-content-between">
          <div className="flex w-74 h-14 sm:w-1/6">
            
            {isMobileView ? <Pet className="ml-0" /> : <Logo size="text-3xl w-12 h-14 p-2" space="space-x-2" />}
          </div>
          {/* Botón de menú móvil (reemplaza al Avatar en dispositivos móviles) */}
          {isMobileView ? (
            <IconButton sx={{ p: 0 }} className="md:hidden" onClick={toggleMobileMenu}>
              {isMobileMenuOpen ? <CloseIcon /> : <MenuIcon />}
            </IconButton>
          ) : (
            <IconButton sx={{ p: 0 }} className="hidden md:flex" /* Solo visible en dispositivos de tamaño medio y grande */>
